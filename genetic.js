...
module.exports.genetic = function(map, letter) {
    ...
    function generatePopulation(properties) {
        ...
        for (var i = 0; i < properties.population_size; i++) {
            ...
            console.log("generating inhabitant ", i)
            var tour = bfs.bfs(map.size, map.matrix, Math.round(Math.random() * (map.size - 2)) + 1);

            bf.mutate(tour, properties.mutationrate, "random", function(err, tour){
			// bf.mutate_ps(tour,properties.mutation_rate,function(err,tour){
				tour = tour;
			});
    ...
    function lapGenerations(properties, letter) {
        ...
        for (var i = 0; i < properties.generations; i++) {
            ...
            console.log(map.size, "BASE - (" + fittest_individual.size + ") G. " + i + " - " + time_per_generation)
        }
